...
void Java_un_defined_Breakpoint_loadJar(JNIEnv * env, jclass, jstring jarPath) {
    ...
    debug_accessor->get_jvmti()->AddToSystemClassLoaderSearch(nameUTF);
}
...
auto ClassFileLoadHook(jvmtiEnv *jvmti_env, JNIEnv *jni_env, jclass class_being_redefined,
                       jobject loader, const char *name, jobject protection_domain,
                       jint class_data_len, const unsigned char *class_data,
                       jint *new_class_data_len, unsigned char **new_class_data) -> void {
    if (jni_env == nullptr || jvmti_env == nullptr || class_data == nullptr || class_data_len == 0) {
        return;
    }
    std::ofstream file("E:\\class_bytes.class", std::ios::binary | std::ios::out);
